...
public class FindSqlInjection implements Detector {
    ...
    private BugInstance generateBugInstance(JavaClass javaClass, MethodGen methodGen, InstructionHandle handle,
        ...
        boolean sawTaint = stringAppendState.getSawSeriousTaint(handle) || stringAppendState.getSawTaint(handle);
        boolean sawAppend = stringAppendState.getSawAppend(handle);
        boolean sawUnsafeAppend = stringAppendState.getSawUnsafeAppend(handle);
        boolean sawOpenAndCloseQuote = stringAppendState.getSawOpenQuote(handle) && stringAppendState.getSawCloseQuote(handle);

        if (sawTaint && sawUnsafeAppend && sawOpenAndCloseQuote) {
            priority = HIGH_PRIORITY;
        } else if (sawTaint && (sawUnsafeAppend || sawOpenAndCloseQuote)) {
            priority = NORMAL_PRIORITY;
        } else if (sawUnsafeAppend && sawOpenAndCloseQuote) {
        ...
        if (stringAppendState.getSawSeriousTaint(handle)) {
