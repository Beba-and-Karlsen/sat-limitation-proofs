...
public class OpcodeStack {
    ...
    private void processMethodCall(DismantleBytecode dbc, int seen) {
        ...
        if ("java/lang/StringBuffer".equals(clsName) || "java/lang/StringBuilder".equals(clsName)) {
        ...
        } else if ("java/lang/String".equals(clsName) && "replaceAll".equals(methodName) && "(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;".equals(signature) && getStackDepth() >= 3) {
            // first.replaceAll(second, third)
            Item third = getStackItem(0);
            Item second = getStackItem(1);
            Item first = getStackItem(2);

            if (first.isServletParameterTainted() || second.isServletParameterTainted() || third.isServletParameterTainted()) {
                servletRequestParameterTainted = true;
            }
            ...
            push(result);
            return;
        } else if (seen == Const.INVOKEVIRTUAL && "getValue".equals(methodName)
            ...
            push(result);
            return;
        } else if (seen == Const.INVOKESTATIC && "getenv".equals(methodName)
            ...
            push(result);
            return;
        } else if (seen == Const.INVOKEVIRTUAL && "getProperty".equals(methodName)
                && "java/util/Properties".equals(clsName)) {
            pop();
            Item result = new Item("Ljava/lang/String;");
            result.setServletParameterTainted();
            result.source = XFactory.createReferencedXMethod(dbc);
            ...
            push(result);
            return;
        } else if (seen == Const.INVOKESTATIC && "getProperty".equals(methodName)
                && "java/lang/System".equals(clsName)) {
            pop();
            Item result = new Item("Ljava/lang/String;");
            result.setServletParameterTainted();
